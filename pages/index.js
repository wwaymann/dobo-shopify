  ...
  try {
    ...
    (c.getObjects() || []).forEach((o) => {
      const visit = (x) => {
        ...
        const keep =
          (kind === "image" && isImg(x)) || (kind === "text" && isTxt(x));
        if (!keep) hide(x);
      };
      visit(o);
    });
...
<img src="${escapeHtml(imgUrl)}" alt=""><h4>${escapeHtml(title || "")}</h4><p>${escapeHtml(
  desc || ""
)}</p>
</div></body></html>`;
...
function Home() {
  ...
  const handlePointerDownCap = (e, ref) => {
    ref.current = {
      btn:
        e.pointerType === "mouse" || e.pointerType === "pen" ? e.button : 0,
      x: e.clientX ?? 0,
      y: e.clientY ?? 0,
    };
  };
  const handlePointerUpCap = (e, ref, handlers) => {
    ...
    if ((e.pointerType === "mouse" || e.pointerType === "pen") && d.btn !== 0)
      return;
  ...
  const applyDesiredSelection = () => {
    ...
    const wantSize = (
      q.get("size") || meta.size || meta.tamano || meta.tamaño || ""
    ).toLowerCase();
    const wantPot = (
      q.get("pot") ||
      q.get("potHandle") ||
  ...
  // DOBO: aplica selección desde meta
  useEffect(() => {
    ...
    const matchIndex = (list, target) => {
      ...
      return list.findIndex((p) => {
        const ids = [p?.id, gidToNum(p?.id), p?.handle, p?.title].map(asStr);
    ...
    if (!userPickedSizeRef.current && sizeRaw) {
      ...
      if (["p", "pequeño", "pequeno", "pequeña", "pequena"].includes(s))
        setActiveSize("Pequeño");
    ...
    const il = matchIndex(
      plants,
      meta.plantId || meta.plant || meta.plantHandle || meta.plantTitle
    );
  ...
  useEffect(() => {
    ...
    const match = (v, c) => {
      ...
      return c
        ? opts.some((o) => lower(o.name) === "color" && lower(o.value) === lower(c))
        : true;
    };
  ...
  async function prepareDesignAttributes() {
    ...
    const pot = pots[selectedPotIndex];
    const plant = plants[selectedPlantIndex];
    const designId = String(Date.now());
    const attrs = [
  ...
  async function publishDesignForVariant(variantId) {
    try {
      ...
      if (!previewDataURL) {
        const canvas =
          api.getCanvas?.() || api.canvas || document.querySelector("canvas");
        try {
          previewDataURL = canvas?.toDataURL?.("image/png") || null;
        } catch {}
      }
      if (!previewDataURL) return { ok: false, error: "no-preview" };

      // meta para restaurar selección
      const meta = {
  ...
  async function waitDesignerReady(timeout = 20000) {
    ...
    while (Date.now() - start < timeout && !resolved) {
      ...
      const ok = a &&
        (a.exportDesignSnapshot || a.exportSnapshot || a.exportJSON || a.toJSON || a.getState);
      if (ok) {
        api = a;
        break;
      }
  ...
  function postCart(shop, mainVariantId, qty, attrs, accessoryIds, returnTo) {
    ...
    const layerImg =
      getA("Layer:Image") || getA("LayerImage") || getA("_LayerImage");
  ...
  const actionCore = useCallback(
    async ({ goCheckout }) => {
      ...
      const isTxt = (o) =>
        /^(i-text|textbox|text)$/i.test(o?.type) || typeof o?.text === "string";
      const isBase = (o) => {
        const tag = String(
      ...
      const pushKV = (k, v) => {
        if (v)
          attrs = [
            ...attrs.filter((a) => a.key !== k && a.key !== `_${k}`),
            { key: k, value: v },
          ];
      };
      ...
      const dpRes = await fetch("/api/design-product", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: `DOBO ${plants[selectedPlantIndex]?.title} + ${
      ...
      const doNum = (
        ...
        .slice(-8)
        .toUpperCase();
      const noNum = String(dp.variantId || "").includes("gid://")
        ? String(dp.variantId).split("/").pop()
        : String(dp.variantId);

      // ——— forzar que DO / _DO / _NO queden en attrs ———
      attrs = (attrs || []).filter((a) => !["DO", "_DO", "_NO"].includes(String(a?.key)));
      if (doNum) {
      ...
      try {
        ...
        }).catch((err) => console.warn("[dobo] handshake failed:", err));
      } catch (err) {
    ...
    (async () => {
      ...
      const wait = async (ms = 20000) => {
        ...
        while (Date.now() - t0 < ms) {
          const a = window.doboDesignAPI;
          const ok =
            a && (a.importDesignSnapshot || a.loadDesignSnapshot || a.loadJSON || a.loadFromJSON);
          if (ok) return a;
          await new Promise((r) => setTimeout(r, 100));
        }
        return null;
      ...
      try {
        ...
        setDesignMeta(designMetaRef.current);
                      ...
                      return (
                        <div
                          ...
                          onMouseMove={(e) =>
                            setAccPreview((p) =>
                              p.visible
                                ? { ...p, x: e.clientX + 16, y: e.clientY + 16 }
                                : p
                            )
                          }
                          onMouseLeave={() =>
              ...
              <div className="d-flex flex-column align-items-center mb-5">
                <div className="mb-3 text-center">
                  ...
                  >
                    ...
                    <button
                      className="btn btn-outline-secondary"
                      onClick={() => setQuantity((p) => Math.min(1000, p + 1))}
                    >
                      +
                    </button>
                  </div>
