// lib/checkout.js
// Cliente: crear carrito por Storefront GraphQL y, si faltan envs, degradar a /cart/add sin romper la UI.
...
function formPostCartAdd(shop, lines = [], returnTo = "/checkout") {
...
export async function cartCreateAndRedirect(lines, cartAttributes = []) {
  ...
  if (!token) {
    console.warn("[DOBO] Sin token público; degradando a /cart/add");
    if (typeof window !== "undefined") formPostCartAdd(shop, lines, "/checkout");
    return null;
  }
  ...
  if (!r.ok || gqlErrors.length || userErrors.length) {
    const msg =
      ...
      `HTTP ${r.status} cartCreate`;
    console.warn("[DOBO] cartCreate falló:", msg, "→ /cart/add");
    if (typeof window !== "undefined") formPostCartAdd(shop, lines, "/checkout");
    return null;
  }
  ...
  if (!checkoutUrl) {
    console.warn("[DOBO] cartCreate sin checkoutUrl → /cart/add");
    if (typeof window !== "undefined") formPostCartAdd(shop, lines, "/checkout");
    return null;
  }
