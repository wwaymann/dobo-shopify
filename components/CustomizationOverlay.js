...
export default function CustomizationOverlay({
  ...
  // Top del área de edición (más alto en móvil)
  const [overlayTop, setOverlayTop] = useState(320);
  useEffect(() => {
    const updateTop = () => setOverlayTop(window.innerWidth <= 768 ? 220 : 320);
    updateTop();
  ...
  useEffect(() => {
    ...
    // Afinar para móvil/iOS
    const upper = c.upperCanvasEl;
    const lower = c.lowerCanvasEl;
    [upper, lower].forEach(el => {
      if (!el) return;
      el.style.touchAction = 'none';                 // evita que Safari "se robe" el gesto
  ...
  useEffect(() => {
    ...
    const setAll = (on) => {
      ...
      if (upper) {
        ...
        upper.style.touchAction = on ? 'none' : 'auto'; // fuerza captura táctil en móvil
  ...
  // Evitar que el host bloquee eventos del canvas en móvil (fallback sin composedPath)
  useEffect(() => {
  ...
  // Bloquea scroll mientras se edita (necesario en iOS)
  useEffect(() => {
    const c = fabricCanvasRef.current;
  ...
  const detachDebossEvents = (o) => {
    try { if (o && o._debossSync) ['changed','modified','moving','scaling','rotating','resizing'].forEach(ev => o.off(ev, o._debossSync)); } catch {}
  };
  ...
  const collectTextFamily = (c, base) => {
    ...
    set.add(base);
    if (base._debossers) { const { shadow, highlight } = base._debossers; if (shadow) set.add(shadow); if (highlight) set.add(highlight); }
    if (fam) all.forEach(o => { if (o && (o._debossOwnerId === fam || o.__ownerId === fam)) set.add(o); });
  ...
  /* ----- Overlay Canvas (escala con la escena) ----- */
  const OverlayCanvas = (
    <div
      ref={overlayRef}
      style={{
    ...
    >
      <canvas
        ...
        style={{
          ...
          touchAction: 'none',            // crítico en móvil
          msTouchAction: 'none',
  ...
  function Menu() {
    return (
      ...
      >
        ...
        {/* Inputs ocultos (gesto de usuario → OK en iOS) */}
