...
/**
 * ===========================================
 * CustomizationOverlay (menu visible siempre)
 * - Menú fijo con zIndex máximo sobre el tema
 * - Undo/Redo con snapshots JSON
 * - Doble click para editar texto (desktop)
 * - Objetos “relief” serializables
 * ===========================================
 */
...
const Z_CANVAS = 4000;
const Z_MENU = 2147483647; // por encima de cualquier sección del tema
...
/** Texto con relieve (grupo de 3 Textbox) serializable */
...
/** Imagen vectorizada con relieve (grupo de 3 Image) serializable */
...
export default function CustomizationOverlay({
  ...
  // Historial
  const undoRef = useRef([]); // pila de “pasados”
  const redoRef = useRef([]); // pila de “futuros”
  ...
  const canUndo = () => undoRef.current.length > 1; // hay un estado previo real
  ...
  const doUndo = () => {
    ...
    undoRef.current.pop(); // quita el actual
  ...
  useEffect(() => {
    ...
    // Doble click (desktop) para editar texto
  ...
  /* ------ Centro del anchor para posicionar menú ------ */
  useLayoutEffect(() => {
    const el = anchorRef?.current;
  ...
  /* ------ Bloquear clics externos mientras se diseña ------ */
  useEffect(() => {
  ...
  return (
    <>
      {/* Overlay dentro del stage, posicionado sobre anchor */}
      {stageRef?.current ? createPortal(OverlayCanvas, stageRef.current) : null}

      {/* Menú fijo centrado abajo. Sin depender de "mounted" */}
