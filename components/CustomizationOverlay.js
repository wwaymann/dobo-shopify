...
// === HOTFIX: forzar que lo cargado quede editable (sin cambiar nada más) ===
useEffect(() => {
  if (!ready) return;
  const c = fabricCanvasRef.current;
  if (!c) return;
  ...
  const makeEditable = () => {
    try {
      // habilitar selección en el lienzo
      ...
      // normalizar cada objeto existente
      (c.getObjects?.() || []).forEach((o0) => {
        let o = o0;

        // Si viniera como 'text', convertir a Textbox editable (mantiene pose)
        ...
        // Hacer cualquier objeto seleccionable/editable
        o.selectable = true;
        o.evented = true;
        o.hasControls = true;
        o.hasBorders = true;
  ...
  // 1) Al estar listo el canvas
  makeEditable();

  // 2) Y cada vez que se cargue un snapshot (loadFromJSON → render)
  let ranOnceAfterLoad = false;
  const onAfterRender = () => {
    if (ranOnceAfterLoad) return;
    ranOnceAfterLoad = true;
    makeEditable();
    // pequeño reset del flag para futuros loads
    setTimeout(() => { ranOnceAfterLoad = false; }, 300);
  };
  c.on('after:render', onAfterRender);

  return () => { try { c.off('after:render', onAfterRender); } catch {} };
}, [ready]);
