...
export default function CustomizationOverlay({
  stageRef,        // nodo con transform: scale(var(--zoom))
  anchorRef,       // carrusel de macetas (lo usamos solo para centrar spawn)
  visible = true,
  zoom,            // opcional (si no viene, trabajamos igual)
  setZoom,         // opcional
}) {
  /** ----- Refs/estado ----- */
  ...
  // tamaño del stage (SIN aplicar scale; es el clientWidth/Height reales)
  const [stageSize, setStageSize] = useState({ w: 1, h: 1 });

  // caja del carrusel dentro del stage (solo para centrar spawn)
  ...
  /** ===== Medición (simple y robusta) =====
   * 1) El overlay cubre TODO el stage → sin desalineo con el scale().
   * 2) Calculamos anchorBox solo para saber dónde centrar texto/imagen al crear.
   */
  const measure = () => {
    const S = stageRef?.current;
    if (!S) return;
    setStageSize({ w: Math.max(1, S.clientWidth), h: Math.max(1, S.clientHeight) });

    const A = anchorRef?.current;
    if (A) {
      const sr = S.getBoundingClientRect();
      const ar = A.getBoundingClientRect();
      // Como el overlay vive dentro del S y S tiene transform:scale,
      // convertimos ar->coords locales dividiendo por escala efectiva.
      // Si no nos pasan zoom, usamos 1 (el error aquí ya no afecta al overlay,
      // solo al spawn inicial).
      const s = typeof zoom === "number" ? zoom : Number(getComputedStyle(S).getPropertyValue("--zoom")) || 1;
      setAnchorBox({
  ...
  useLayoutEffect(() => {
    ...
    try { roS.observe(S); } catch {}
    let roA;
    if (A) { roA = new ResizeObserver(measure); try { roA.observe(A); } catch {} }
    const onWin = () => measure();
  ...
  useEffect(() => { measure(); }, [zoom]); // si cambias zoom desde fuera
  ...
  /** ===== Deboss agrupado (sin separación de capas) ===== */
  const syncImgOffsets = (group) => {
  ...
  const makeImageDebossGroup = (element, { left, top, originX = "center", originY = "center", scale = 1, angle = 0 } = {}) => {
    ...
    const group = new fabric.Group([shadow, highlight, base], {
      originX, originY, left, top, angle, scaleX: scale, scaleY: scale, objectCaching: false, selectable: true, evented: true,
    });
    group._kind = "imgDeboss";
    group._vecSourceEl = element;
    group._debossChildren = { shadow, highlight, base };
    syncImgOffsets(group);
    const bind = () => { syncImgOffsets(group); group.canvas?.requestRenderAll(); };
    ["modified", "moving", "scaling", "rotating", "skewing"].forEach(ev => group.on(ev, bind));
    return group;
  };
  ...
  const revectorizeImageGroup = (group) => {
    ...
    const fresh = new fabric.Group([newShadow, newHighlight, newBase], {
      originX, originY, left, top, angle, scaleX, scaleY, objectCaching: false, selectable: true, evented: true,
    });
    ...
    syncImgOffsets(fresh);
    ["modified", "moving", "scaling", "rotating", "skewing"].forEach(ev => fresh.on(ev, () => { syncImgOffsets(fresh); c.requestRenderAll(); }));
    c.add(fresh); c.setActiveObject(fresh); c.requestRenderAll();
  };
  ...
  const makeTextDebossGroup = (text, opts) => {
    ...
    ["modified", "moving", "scaling", "rotating", "skewing", "changed"].forEach(ev => group.on(ev, bind));
    return group;
  };
  ...
  useEffect(() => {
    ...
    if (upper) {
      upper.style.pointerEvents = "auto";   // para mouse
      upper.style.touchAction = "none";     // para touch/pinch
      upper.tabIndex = 0;
    }

    // Doble click → editar el hijo base del grupo de texto
    ...
    const classify = (t) => (!t ? "none" : t._kind === "textDeboss" ? "text" : t._kind === "imgDeboss" ? "image" : "none");
  ...
  // Redimensionar lienzo = tamaño del stage (overlay cubre todo)
  useEffect(() => {
    const c = fabricCanvasRef.current;
    if (!c) return;
    c.setWidth(stageSize.w);
    c.setHeight(stageSize.h);
    c.calcOffset?.();
    c.requestRenderAll?.();
  }, [stageSize.w, stageSize.h]);

  /** ===== Alternar edición (clics de mouse fuera/ dentro) ===== */
  useEffect(() => {
    const c = fabricCanvasRef.current;
    if (!c) return;
    const on = !!editing;
    ...
    (c.getObjects?.() || []).forEach(o => { o.selectable = on; o.evented = on; o.lockMovementX = !on; o.lockMovementY = !on; });
    const upper = c.upperCanvasEl;
  ...
  /** ===== Zoom con rueda/pinch SOLO en “Diseñar” ===== */
  useEffect(() => {
    const c = fabricCanvasRef.current;
    const upper = c?.upperCanvasEl;
    const S = stageRef?.current;
    ...
    const readZoom = () => {
      const z = typeof zoom === "number" ? zoom : Number(getComputedStyle(S).getPropertyValue("--zoom")) || 1;
      return clamp(z, 0.8, 2.5);
    };
    const writeZoom = (z) => {
      const val = clamp(z, 0.8, 2.5);
      if (typeof setZoom === "function") setZoom(val);
      else S.style.setProperty("--zoom", String(val));
      measure();
    };
    ...
    // pinch 2 dedos
    const pts = new Map();
    let startDist = 0, startScale = readZoom();
    ...
    const onPM = (e) => {
      ...
      writeZoom(startScale * Math.pow(factor, 0.9));
    };
    const onPU = (e) => {
      if (e.pointerType !== "touch") return;
      pts.delete(e.pointerId);
      if (pts.size < 2) { startDist = 0; startScale = readZoom(); }
    };
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [editing, zoom, setZoom]);

  /** ===== Acciones ===== */
  const spawnCenter = () => {
    if (anchorBox) return { x: anchorBox.left + anchorBox.w / 2, y: anchorBox.top + anchorBox.h / 2 };
    return { x: stageSize.w / 2, y: stageSize.h / 2 };
    };
  ...
  const addText = () => {
    ...
    const group = makeTextDebossGroup("Nuevo párrafo", {
      ...
      width: Math.min(stageSize.w * 0.9, 240),
  ...
  const addImageFromFile = (file) => {
    ...
    imgEl.onload = () => {
      ...
      const maxW = stageSize.w * 0.6, maxH = stageSize.h * 0.6;
      const s = Math.min(maxW / (sample.width || 1), maxH / (sample.height || 1), 1);
      const p = spawnCenter();
      const group = makeImageDebossGroup(src, { left: p.x, top: p.y, originX: "center", originY: "center", scale: s, angle: 0 });
  ...
  const replaceActiveFromFile = (file) => {
    ...
    const url = URL.createObjectURL(file);
    const imgEl = new Image(); imgEl.crossOrigin = "anonymous";
    imgEl.onload = () => { t._vecSourceEl = downscale(imgEl); revectorizeImageGroup(t); URL.revokeObjectURL(url); };
    imgEl.onerror = () => URL.revokeObjectURL(url);
    imgEl.src = url;
  };
  ...
  const clearSelection = () => {
    const c = fabricCanvasRef.current; if (!c) return;
    try { c.discardActiveObject(); c.setActiveObject(null); c._activeObject = null; } catch {}
    setSelType("none"); c.requestRenderAll();
  };
  const enterDesignMode = () => { clearSelection(); setEditing(true); };
  const exitDesignMode  = () => { clearSelection(); setEditing(false); };
  ...
  useEffect(() => {
    if (!editing || selType !== "image") return;
    const a = fabricCanvasRef.current?.getActiveObject();
    if (a && a._kind === "imgDeboss") { syncImgOffsets(a); fabricCanvasRef.current.requestRenderAll(); }
  }, [vecOffset, editing, selType]);

  if (!visible) return null;

  /** ===== Overlay: cubre TODO el stage (sin desalineos) ===== */
  const OverlayCanvas = (
    <div
      style={{
        position: "absolute",
        inset: 0,
        zIndex: Z_CANVAS,
        pointerEvents: editing ? "auto" : "none", // fuera de "Diseñar" → los clics del mouse pasan al carrusel
  ...
  function Menu() {
    const S = stageRef?.current;
    const z = typeof zoom === "number" ? zoom : Number(S && getComputedStyle(S).getPropertyValue("--zoom")) || 1;
    const zVal = Math.round(clamp(z, 0.8, 2.5) * 100);

    const writeZoom = (nv) => {
      const val = clamp(nv, 0.8, 2.5);
      if (typeof setZoom === "function") setZoom(val);
      else S?.style.setProperty("--zoom", String(val));
      measure();
    };
      ...
      >
        <div style={{ display: "flex", justifyContent: "center", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <div className="input-group input-group-sm" style={{ width: 180 }}>
            ...
            <button type="button" className="btn btn-outline-secondary" onClick={() => writeZoom(z - 0.1)}>−</button>
